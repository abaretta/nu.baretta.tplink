    ...
    */
    ...
    light_temperature: {
        ...
        set: function (device_data, light_temperature, callback) {
            ...
            if (light_temperature == 0) {
                tempLevel = light_temperature;
            } else {
...
function circadianModeOn(device) {
    bulb = client.getBulb({
        host: device.id
    });
    options = {
...
function circadianModeOff(device) {
    Homey.log('TP Link smartbulb app - circadionModeOn device: ' + device.id);
    bulb = client.getBulb({
        host: device.id
    });
    options = {
...
function onTransition(device, transition) {
    bulb = client.getBulb({
        host: device.id
    });
    options = {
...
function offTransition(device, transition) {
    bulb = client.getBulb({
        host: device.id
    });
    options = {
    ...
    try {
        ...
        bulb.getInfo().then((data) => {
            ...
            if (data.lighting.lightState.on_off === 1) {
                ...
                // updated states
                if (data.lighting.lightState.color_temp == 0) {
                    device.state.light_temperature == 0
                } else {
