...
var commands = {
  ...
  resetConsumption: () => `{"emeter":{"erase_emeter_stat":null}}`,
...
class Plug extends EventEmitter {
  ...
  set sysInfo (sysInfo) {
   ...
   // the following fails for light bulbs, causing a crash.
   // this.supportsConsumption = (sysInfo.feature.includes('ENE'));
   // only LB100 and HS100 do not support energy monitoring
    if(this.model.indexOf("100") > 0) {
  ...
  resetConsumption () {
    var cmd = commands.resetConsumption();
    return this.set(cmd).then((data) => {
      try {
        var errCode = data.system.erase_emeter_stat.err_code;
        return (errCode === 0);
      } catch (e) {}
      if (errCode !== 0) { throw data; }
    });
